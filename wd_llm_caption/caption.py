...
class Caption:
    ...
    def download_models(
        ...
        self.my_logger.info(f"Caption method: {args.caption_method}")
        self.my_logger.info(f"Run method: {args.run_method}")
        # Set flags
        ...
        if self.use_wd and args.caption_method == "wd+llm":
            ...
            if args.run_method == "sync":
                ...
                if self.use_joy:
                    ...
                    pbar.set_description(f'Processing with Online model({args.llm_model_name})...')
...
def setup_args() -> argparse.Namespace:
    ...
    llm_args.add_argument(
        '--llm_online_api_key',
        type=str,
        help='API Key for online LLM Caption models.'
    )
    llm_args.add_argument(
        '--llm_online_base_url',
        type=str,
        help='Base url for online LLM Caption models.'
    )
    # llm_args.add_argument(
    #     '--llm_online_endpoint',
    #     type=str,
    #     help='Endpoint for online LLM Caption models. (Current Only Doubao need this)'
    # )
    llm_args.add_argument(
        '--llm_model_name',
        type=str,
    ...
    llm_args.add_argument(
        ...
        type=float,
        default=0,
        help='top_p for LLM model, default is `0`，means use llm own default value.'
    )
    llm_args.add_argument(
        '--llm_max_tokens',
        type=int,
